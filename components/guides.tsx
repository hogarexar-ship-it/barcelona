...
export function GuideCards({ guides, columns = 3 }: { guides: Guide[]; columns?: 2 | 3 }) {
  return (
    <div className={`grid gap-6 sm:grid-cols-2 ${columns === 3 ? "lg:grid-cols-3" : ""}`}>
      {guides.map((guide) => (
        <Link
          ...
          className="group flex flex-col rounded-xl2 border border-ink-100 bg-white p-6 transition hover:border-accent-200 hover:shadow-lg"
        >
          <span className="text-xs font-semibold uppercase tracking-wide text-accent-600">{guide.category}</span>
          <h3 className="mt-2 font-display text-lg font-bold text-ink-900 group-hover:text-accent-600">{guide.title}</h3>
          <p className="mt-2 flex-1 text-sm text-ink-600">{guide.excerpt}</p>
          <time dateTime={guide.publishedAt} className="mt-4 text-xs text-ink-400">
  ...
  return (
    <>
      ...
      <div className="bg-surface-100 pt-6">
        <Container>
          <Breadcrumbs items={[...crumbs, { name: guide.title, href: guidePath(guide) }]} />
        </Container>
      </div>

      <article className="py-12 sm:py-16">
        <Container className="max-w-2xl">
          <span className="eyebrow">{pro ? `Para profesionales · ${guide.category}` : guide.category}</span>
          ...
          <div className="mt-8 rounded-xl2 border border-accent-200 bg-accent-50 p-6">
            <p className="text-sm font-semibold uppercase tracking-wide text-accent-700">En resumen</p>
          ...
          {pro ? (
            <div className="mt-14 rounded-xl2 bg-ink-900 p-7 text-white sm:p-9">
              ...
              <ProCta href={guide.trade ? `${proRoutes.join}?oficio=${guide.trade}` : proRoutes.join} className="mt-6" />
              {guide.trade && (
                <p className="mt-5 text-sm text-ink-100">
                  <Link href={sectorPath(guide.trade)} className="font-semibold text-white underline underline-offset-4">
                    Cómo ayudamos a {service?.professionalPlural ?? "profesionales"}
                  </Link>
                </p>
              )}
            </div>
            ...
            <div className="mt-14 rounded-xl2 bg-surface-100 p-7 sm:p-9">
              <p className="font-display text-2xl font-bold text-ink-900">
                {service ? `¿Necesitas un ${service.professional} en Barcelona?` : "¿Necesitas un profesional en Barcelona?"}
              </p>
              <p className="mt-3 text-ink-600">
                Cuéntanos qué pasa y te ponemos en contacto con un profesional verificado de tu zona. Gratis y sin compromiso.
              </p>
              <PrimaryCta href={requestHref(guide.trade)} className="mt-6" />
              {guide.trade && (
                <p className="mt-5 text-sm text-ink-600">
                  <Link href={servicePath(guide.trade)} className="font-semibold text-ink-900 underline underline-offset-4">
