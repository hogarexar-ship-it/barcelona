...
const title = "Marketing para empresas de reformas, fontaneros y electricistas en Barcelona";
const description =
...
export default function MarketingPage() {
  return (
    <>
      <JsonLd data={serviceSchema({ name: title, description, url: `${siteConfig.url}${path}` })} />
      <JsonLd data={faqSchema(faqs)} />

      <div className="bg-cream-100 pt-6">
        <Container>
          <Breadcrumbs items={[{ name: "Marketing para profesionales", href: path }]} />
        </Container>
      </div>
      ...
      <PageHero
        ...
        subtitle="Llevamos el marketing de tu negocio de reformas, fontanería o electricidad en Barcelona: Google, anuncios, web, marca y redes. Tú eliges qué necesitas."
        actions={
          <>
            <PrimaryCta href="/empezar?interes=marketing" label={marketingOffer.cta} />
            <WhatsAppButton />
          </>
        }
      />
      ...
      <section id="diagnostico" className="scroll-mt-20 py-16 sm:py-24">
        <Container>
          <div className="grid gap-10 rounded-xl2 border border-terracotta-200 bg-terracotta-50 p-7 sm:p-10 lg:grid-cols-2 lg:items-center">
            <div>
              ...
              <PrimaryCta href="/empezar?interes=marketing" label="Pedir mi diagnóstico" className="mt-8" />
            </div>
          ...
          <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {marketingServices.map((service) => (
              <article key={service.name} className="rounded-xl2 border border-ink-100 bg-white p-6">
                ...
                <p className="mt-5 text-sm font-semibold uppercase tracking-wide text-terracotta-600">{service.name}</p>
      ...
      <section className="bg-cream-100 py-16 sm:py-24">
        <Container className="grid gap-10 lg:grid-cols-2 lg:items-center">
          <SectionHeading
            ...
            intro="No somos una agencia generalista. Sabemos qué busca quien necesita un fontanero a las nueve de la noche o quien compara tres presupuestos de reforma, y diseñamos tu marketing para eso."
          />
          <div className="rounded-xl2 bg-ink-900 p-7 text-white sm:p-9">
            <p className="text-sm font-semibold uppercase tracking-wide text-terracotta-300">¿Necesitas trabajo ya?</p>
